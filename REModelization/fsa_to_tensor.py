...
def dfa_to_tensor(automata, word2idx: Dict[str, int], subtype=False, disturb_func=subtype_disturb_func):
    ...
    for fr_state, to in sorted(automata['transitions'].items()):
        # print('fr_state: {}, to: {}'.format(fr_state, to))
...
class Automata:
    ...
    def getReachableStates(self, states: Union[int, List[int]], token: str):
        """
        Gets all the states that can be reached by a given character in a state.
        """
